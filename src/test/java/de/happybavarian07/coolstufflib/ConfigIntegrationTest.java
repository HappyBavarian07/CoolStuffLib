...
class ConfigIntegrationTest {
    ...
    @Test void testFullConfigLifecycle() {
    ...
    @Test void testBackupRetention() {
        ...
        module.onAttach(config);
        module.save();
        try (FileWriter writer = new FileWriter(configFile)) { writer.write("k: v2"); } catch (IOException e) { fail(); }
        module.save();
        try (FileWriter writer = new FileWriter(configFile)) { writer.write("k: v3"); } catch (IOException e) { fail(); }
        module.save();
        assertTrue(module.listBackups().size() <= 2);
    }

    @Test void testErrorScenarios() {
        ...
        boolean restored = module.restoreBackup(99);
        assertFalse(restored);
        // Try deleting non-existent backup
        boolean deleted = module.deleteBackup(99);
        assertFalse(deleted);
    }
}